...
export default function TabXOrm({ caption, captionLoading, captionError }: CaptionProps) {
  const [copiedTags, setCopiedTags] = useState(false);
  const [copiedCaptionWithTags, setCopiedCaptionWithTags] = useState(false);
  const copy = useCopyToClipboard();

  const tags = X_TAGS.orm;
  ...
  return (
    <>
      <Card title="Tags">
        <p className="mb-4 text-sm text-white/80">Copy hashtags for your post (Orm).</p>
